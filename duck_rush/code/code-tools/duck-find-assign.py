# -*- coding:utf-8 -*-
# @author xupingmao
# @filename duck-find-assign.py
# @description 搜索对某个名字的赋值操作。自动把名字转为 camelCase / snake_case /
#              PascalCase / SCREAMING_SNAKE 等多种形式并大小写不敏感匹配; 同时支持
...
def highlight(line: str, pattern) -> str:
    """高亮行内第一个命中区间。"""
...
def print_results(results, show_label: bool, line_number: bool,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="搜索对某个名字的赋值操作(自动驼峰/下划线互转, 大小写不敏感, "
                    "支持 setXxx setter)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "命名变体: 输入 userName / user_name 都会同时匹配 userName、UserName、\n"
            "user_name、USER_NAME, 且大小写不敏感; 同时匹配 setUserName(...) / set_user_name(...)。\n\n"
            "支持语言(按扩展名自动识别):\n"
...
def main() -> None:
    ...
    if args.lang is not None and args.lang not in list_lang_names():
        sys.stderr.write("duck-find-assign: 未知语言 '%s' (支持: %s)\n"
    ...
    if "-" in targets and sys.stdin.isatty():
        sys.stderr.write(
            "duck-find-assign: 未通过管道传入数据(stdin 为终端)\n"
            "用法: cat 文件 | duck-find-assign NAME -\n")
        sys.exit(2)

    finder = AssignmentFinder(
        args.name,
        setters=not args.no_setters,
    ...
    if not results:
        sys.stderr.write("duck-find-assign: 未找到对 '%s' 的赋值\n" % args.name)
    else:
        sys.stderr.write("duck-find-assign: 在 %d 个文件中共 %d 处命中\n"
                         % (len(results), total))
